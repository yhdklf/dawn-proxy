...
proxy = config.get("http_proxy")  # Add this line to read the proxy
...
def total_points(headers):
    try:
        ...
        response = requests.get(get_points_url, headers=headers, proxies=proxies, verify=False)
...
def keep_alive(headers, email):
    ...
    try:
        response = requests.post(keepalive_url, headers=headers, json=keepalive_payload, proxies=proxies, verify=False)
        response.raise_for_status()
